...
export VEHICLE_NAME=$(jq -r '.vehicle_name // "Mój samochód"' "$CONFIG")
export TANK_CAPACITY_L=$(jq -r '.tank_capacity_l // "50.0"' "$CONFIG")
...
export LOCATION_ENTITY=$(jq -r '.location_entity // ""' "$CONFIG")
export PRICE_REGION=$(jq -r '.price_region // "mazowieckie"' "$CONFIG")
export ODO_BUDGET_ENTITY=$(jq -r '.odo_budget_entity // ""' "$CONFIG")
...
export GEMINI_MODEL=$(jq -r '.gemini_model // ""' "$CONFIG")
export LOCAL_LLM_BASE_URL=$(jq -r '.local_llm_base_url // ""' "$CONFIG")
export LOCAL_LLM_API_KEY=$(jq -r '.local_llm_api_key // ""' "$CONFIG")
export LOCAL_LLM_MODEL=$(jq -r '.local_llm_model // "gemini-3.1-flash-lite"' "$CONFIG")

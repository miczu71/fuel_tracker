...
<tr><td>pomorskie</td><td class="text-center">6,34</td>
...
def test_parse_prices_region_row():
    assert prices.parse_prices(HTML, "pomorskie") == {
...
def test_parse_prices_unknown_region():
    ...
    assert prices.parse_prices("<html></html>", "pomorskie") == {}


def test_store_and_latest_price(conn):
    prices.store_prices(conn, "pomorskie", {"PB95": 6.34, "ON": 6.71},
                        now=datetime(2026, 7, 6))
    latest = prices.latest_price(conn, "pomorskie", "PB95")
    assert latest == {"price": 6.34, "fetched_at": "2026-07-06"}
    assert prices.latest_price(conn, "pomorskie", "LPG") is None
    # nadpisanie tego samego dnia — bez duplikatu
    prices.store_prices(conn, "pomorskie", {"PB95": 6.40},
                        now=datetime(2026, 7, 6))
    assert prices.latest_price(conn, "pomorskie", "PB95")["price"] == 6.40


def test_retention_purges_old_rows(conn):
    conn.execute(
        "INSERT INTO fuel_prices (fetched_at, station, fuel_type, price, source)"
        " VALUES ('2020-01-01', 'region:pomorskie', 'PB95', 5.0, 'autocentrum')")
    prices.store_prices(conn, "pomorskie", {"PB95": 6.34})
...
def test_price_series(conn):
    prices.store_prices(conn, "pomorskie", {"PB95": 6.30},
                        now=datetime(2026, 7, 1))
    prices.store_prices(conn, "pomorskie", {"PB95": 6.34},
                        now=datetime(2026, 7, 6))
    assert prices.price_series(conn, "pomorskie", "PB95") == [
...
def test_fetch_network_failure_returns_empty(monkeypatch):
    ...
    assert prices.fetch_region_prices("pomorskie") == {}
...
def test_sensor_values_price_vs_region(conn):
    ...
    v = queries.sensor_values(conn, vid, 0.0, now=datetime(2026, 7, 6),
                              fuel_type="PB95", price_region="pomorskie")
...
def test_sensor_values_no_region_data(conn):
    vid = dbm.ensure_vehicle(conn, "Testowy", 66.0, "PB95")
    v = queries.sensor_values(conn, vid, 0.0, price_region="pomorskie")
